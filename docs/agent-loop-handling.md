...
>
> **误判修复（2026-08）：`extractPhrases` 只认“散文子句”（通用原则）**。修复前的成界符含代码结构标点（逗号、括号、方括号），Go struct tag 被切成重复片段（5 个 `omitempty` 字段 → 误报 `omitempty"` ×5）；第一版修复误删逗号成界，又导致 `run again, run again` 类拉丁语子句循环漏检。现在的通用规则：① 仅子句级标点（句末标点、逗号、换行、`.`）成界；② 含结构性标点（引号/反引号/括号/方括号/花括号）的片段是代码片段而非子句，不参与计数；③ 纯 ASCII 单 token（标识符）不算重复信号，纯 CJK 短语仍按单 token 计数（该文字无词空格）。效果：struct tag、JSON、调用前缀、`return ""` 尾巴等代码噪声不再误报；逗号分隔的拉丁语子句循环恢复检出。已知残留：无结构标点的多词代码行（如 5 个相同的 `return err`）在窗口内仍会误报，且与真实死循环在字符串层面不可区分。
>
> **默认关闭（2026-08）**：因上述残留本质上需要语言学/LLM 判断（区分“合法代码里 5 个相同行”与“模型复读 5 次”），而字符串启发式无法做到，`loop-guard` 改为**默认关闭**（`defaultMode` 默认 `off`），按会话用 `/loop-guard on` 开启，或经扩展选项 `defaultMode: "on"` 永久开启。
