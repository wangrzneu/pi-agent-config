...
默认开启。Agent 运行期间，循环守卫会跟踪工具调用并检测卡死循环：同一调用连续重复（5 次起，
可配置）、相同的 2/3 步调用循环，或单次 run 超过 120 次工具调用。检测到后询问是否中止当前 run
（print/RPC 模式直接中止）。使用 `/loop-guard` 查看状态，或用 `off|on|reset` 禁用、重新启用或
清零计数。
